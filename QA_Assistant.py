...
class PDFLearningAssistant:
    """docstring for PDFLearningAssistant"""
    ...
    def load_document(self, pdf_path: str):
        ...
        try:
            ...
            return {
                    "success": True, 
                    "message": f"加载成功！(耗时: {process_time:.1f}s)",
    ...
    def ask(self, question: str, use_advanced_search: bool = True):

        if not self.current_document:
            return "请先加载文档！使用load_document()文档"
...
def create_gradio_ui():
    ...
    def chat(message: str, history: List) -> Tuple[str, List]:
        ...
        if assistant_state["assistant"] is None:
            ...
            history.append(ChatMessage(role="assistant", content="❌ 请先初始化助手并加载文档"))
    ...
    def add_note_ui(note_content: str, concept: str):
        """添加笔记"""
    ...
    def generate_report_ui():
        ...
        result += f"-- 会话时长: {report['session_info']['duration_seconds']:.0f}s\n"
    ...
    with gr.Blocks(title="智能文档问答助手", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Tab("💬 智能问答"):
            ...
            msg_input.submit(fn=chat, inputs=[msg_input, history_state], outputs=[msg_input, chatbot, history_state])
            send_btn.click(fn=chat, inputs=[msg_input, history_state], outputs=[msg_input, chatbot, history_state])
