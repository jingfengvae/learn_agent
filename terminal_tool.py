...
class TerminalTool(Tool):
    ...
    def _handle_cd(self, parts):
        ...
        try:
            ...
            return f"X 不允许访问工作目录以外的目录: {new_dir}"

        # 检查目录是否存在
        if not new_dir.exists():
            return f"X 目录不存在: {new_dir}"

        if not new_dir.is_dir:
            return f"X {new_dir}: 不是目录"

        # 更换当前目录
        self.current_dir = new_dir
        return f"切换到目录: {self.current_dir}"    
        
    def _excute_command(self, command):
        """执行命令"""
        try:
            # 根据操作系统类型调整命令执行方式
            if self.os_type == "windows":
                # Windows 下使用 cmd.exe 或者 shell = True
            ...
            if len(output) > self.max_output_size:
                ...
                output += f"\n\n 输出被截断: (超过最输出{self.max_output_size}字节)"
            ...
            return (f"X 命令执行超时: 超过{self.timeout}s, Error: {e}")
        except Exception as e:
            return f"命令执行失败: {e}"
    ...
    def run(self, parameters: Dict[str, Any]):
        ...
        if base_command not in self.ALLOWED_COMMANDS:
            return f"X 不允许的命令: {base_command} \n"
        ...
        # 执行命令
        return self._excute_command(command)
